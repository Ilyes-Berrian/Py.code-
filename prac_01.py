import pygame
import random
import time
import os

# Initialize Pygame
pygame.init()

# Set up the game window
WINDOW_WIDTH = 800
WINDOW_HEIGHT = 600
BLOCK_SIZE = 20
GAME_SPEED = 15

# Colors
BLACK = (20, 20, 100)
WHITE = (255, 255, 255)
RED = (255, 0, 0)
GREEN = (0, 255, 0)

# Create the game window
window = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
pygame.display.set_caption('Snake Game')

# Set icon if available
try:
    if os.path.exists("snake_icon.ico"):
        icon = pygame.image.load("snake_icon.ico")
        pygame.display.set_icon(icon)
except pygame.error as e:
    print(f"Could not load icon: {e}")

# Initialize clock for controlling game speed
clock = pygame.time.Clock()

class Snake:
    def __init__(self):
        self.length = 1
        self.positions = [(WINDOW_WIDTH//2, WINDOW_HEIGHT//2)]
        self.direction = random.choice([UP, DOWN, LEFT, RIGHT])
        self.color = GREEN
        self.score = 0
        self.stopwtach=0

    def get_head_position(self):
        return self.positions[0]

    def update(self):
        cur = self.get_head_position()
        x, y = self.direction
        new = ((cur[0] + (x*BLOCK_SIZE)) % WINDOW_WIDTH, (cur[1] + (y*BLOCK_SIZE)) % WINDOW_HEIGHT)
        if new in self.positions[3:]:
            return False
        else:
            self.positions.insert(0, new)
            if len(self.positions) > self.length:
                self.positions.pop()
            return True

    def reset(self):
        self.length = 1
        self.positions = [(WINDOW_WIDTH//2, WINDOW_HEIGHT//2)]
        self.direction = random.choice([UP, DOWN, LEFT, RIGHT])
        self.score = 0
        self.stopwtach=0
        
    def render(self, surface):
        for p in self.positions:
            pygame.draw.rect(surface, self.color, (p[0], p[1], BLOCK_SIZE, BLOCK_SIZE))

class Food:
    def __init__(self):
        self.position = (0, 0)
        self.color = RED
        self.randomize_position()

    def randomize_position(self):
        self.position = (random.randint(0, (WINDOW_WIDTH-BLOCK_SIZE)//BLOCK_SIZE) * BLOCK_SIZE,
                        random.randint(0, (WINDOW_HEIGHT-BLOCK_SIZE)//BLOCK_SIZE) * BLOCK_SIZE)

    def render(self, surface):
        pygame.draw.rect(surface, self.color, (self.position[0], self.position[1], BLOCK_SIZE, BLOCK_SIZE))

# Directional constants
UP = (0, -1)
DOWN = (0, 1)
LEFT = (-1, 0)
RIGHT = (1, 0)

def main():
    snake = Snake()
    food = Food()
    running = True
    game_over = False
    paused = False
    pin = False
    font = pygame.font.Font(None, 36)  # Moved font initialization here
    while running:
            snake.stopwtach+=1
            if snake.stopwtach != 60:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        running = False
                    elif event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_p:  # P key to pause
                            paused = not paused
                        elif not paused :  # Only handle other keys if not paused
                            if game_over:
                                snake.reset()
                                food.randomize_position()
                                game_over = False
                            else:
                                if event.key == pygame.K_UP and snake.direction != DOWN:
                                    snake.direction = UP
                                elif event.key == pygame.K_DOWN and snake.direction != UP:
                                    snake.direction = DOWN
                                elif event.key == pygame.K_LEFT and snake.direction != RIGHT:
                                    snake.direction = LEFT
                                elif event.key == pygame.K_RIGHT and snake.direction != LEFT:
                                    snake.direction = RIGHT

                if not game_over and not paused:
                    if not snake.update():
                        game_over = True

            # Check if snake ate the food
                    if snake.get_head_position() == food.position:
                            snake.length += 1
                            if (snake.length % 4) == 0 :
                                if event.key == pygame.K_1:
                                    if event.key ==pygame.K_8: 
                                        if event.key ==pygame.K_0:
                                            if event.key ==pygame.K_6:
                                                if event.key % 6 == 0:
                                                    snake.score += 100
                            else:
                                snake.score += 1    
                    food.randomize_position()

        # Draw everything
                window.fill(BLACK)
                snake.render(window)
                food.render(window)
        
        # Display score
                score_text = font.render(f'Score: {snake.score}', True, WHITE)
                window.blit(score_text, (10, 20))
                stopwatch_text= score_text = font.render(f'Stop watch: {snake.stopwtach}', True, WHITE)
                window.blit(score_text, (10, 40)) 
                if game_over:
                    game_over_text = font.render('Game Over! Press any key to restart', True, WHITE)
                    window.blit(game_over_text, (WINDOW_WIDTH//2 - 200, WINDOW_HEIGHT//2))
            else:        
                paused= True
            pygame.display.update()
            clock.tick(GAME_SPEED)

    pygame.quit()

if __name__ == "__main__":
    main()